import { X, MessageSquareWarning, FileText, Clock } from "lucide-react";
import type { Revision } from "@shared/services/actions/evaluador";

interface SolicitarRevisionModalProps {
  modalRevision: Revision | undefined;
  obs: string;
  onObs: (v: string) => void;
  onClose: () => void;
  onSubmit: () => void;
...
export function SolicitarRevisionModal({
  modalRevision,
  obs,
  onObs,
  onClose,
      ...
      >
        ...
        <div className="p-5 flex flex-col gap-4">
          {modalRevision ? (
            <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-edu-control bg-edu-subtle text-[0.8125rem]">
              ...
              <span className="text-edu-ink-700 font-medium">
                {modalRevision.materia} · {modalRevision.seccion}
              </span>
              <span className="text-edu-ink-400">— {modalRevision.docente}</span>
            </div>
          ) : null}
          ...
          <div className="flex gap-2 justify-end">
            ...
            >
              <Clock className="w-4 h-4" /> Enviar solicitud
            </button>
          </div>
        </div>
      </div>
