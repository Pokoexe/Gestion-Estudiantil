...
type TipoPersona = "Docente" | "Estudiante";
...
export function CoordIncidenciasPage() {
    ...
    const visibles = incidencias.filter((i) => filtro === "Todos" || i.tipo === filtro);
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            <div className="flex gap-2">
                {(["Todos", "Docente", "Estudiante"] as const).map((f) => (
                    <button
                        key={f}
                        onClick={() => setFiltro(f)}
                        className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-edu-pill text-[0.8125rem] font-semibold cursor-pointer transition-colors border ${
                            filtro === f ? "text-white border-transparent" : "bg-edu-surface text-edu-ink-700 border-edu-border hover:bg-edu-subtle"
                        }`}
                        style={filtro === f ? { backgroundColor: accent.purple.fg } : undefined}
                    >
                        {f === "Docente" && <UserSquare2 style={{ width: 14, height: 14 }} />}
                        {f === "Estudiante" && <GraduationCap style={{ width: 14, height: 14 }} />}
                        {f === "Todos" ? "Todos" : f + "s"}
                    </button>
                ))}
            </div>
            ...
            <div className="bg-edu-surface rounded-edu-card border border-edu-border-soft overflow-hidden">
                <div className="px-5 py-4 border-b border-edu-border-soft flex justify-between items-center">
                    ...
                    <span className="text-[0.8rem] text-edu-ink-400 font-medium">{visibles.length} incidencias</span>
                </div>
                <div>
                    ...
                    {visibles.length === 0 ? (
                        <p className="text-[0.8125rem] text-edu-ink-400 m-0 px-5 py-6 text-center">No hay incidencias para este filtro.</p>
                    ) : (
                        visibles.map((inc, i) => {
                            const st = GRAVEDAD_META[inc.gravedad];
                            return (
                                <div key={inc.id} className={`grid ${COLS} px-5 py-[13px] items-center transition-colors hover:bg-edu-subtle ${i < visibles.length - 1 ? "border-b border-edu-border-soft" : ""}`}>
            ...
            {/* Formato de incidencias (configurable, solo visual) */}
            <div className="bg-edu-surface rounded-edu-card border border-edu-border-soft overflow-hidden">
                <div className="px-5 py-4 border-b border-edu-border-soft flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    {(["Estudiante", "Docente"] as TipoPersona[]).map((t) => (
                                        <button
                                            ...
                                            className={`flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-edu-control text-[0.8125rem] font-semibold cursor-pointer transition-colors border ${
                                                form.tipo === t ? "text-white border-transparent" : "bg-edu-surface text-edu-ink-700 border-edu-border hover:bg-edu-subtle"
                                            }`}
